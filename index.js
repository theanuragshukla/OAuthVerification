...
const {checkSpaces, checklen, checkPass, checkName, checkEmail} = require('some-random-form-validator');
...
app.post('/create-new-app',async(req,res)=>{
    ...
    if(!checkSpaces(name,true) || !checklen(6,50,name)){
        res.status(400).json({status:false,msg:"invalid App name"})
        return
    }
    const dupquery = `SELECT * FROM apps WHERE appname = $1;`;
    const dupvalue = [req.body.name];
    const dups = await db.query(dupquery, dupvalue);
    if( dups.rows.length!=0){
        res.status(401).json({status:false,msg:"app nama already in use"})
        return
    }
    const query = `
